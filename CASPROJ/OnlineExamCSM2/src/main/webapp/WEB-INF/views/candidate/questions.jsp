...
<script type="text/javascript">
        ...
        function startInactivityTimer() {
            inactivityTimer = setTimeout(function() {
                ...
                //redirectToLogin(); 
            }, 60000); // 3 minutes = 180,000 milliseconds
        }
        //goToQuestion();
...
var IDlist = [];
 //alert(!JSON.stringify(${listOfIDS}) === undefined);
//alert(JSON.stringify(${listOfIDS})+"   -  value of ListOFIDS"); 
//alert(${listOfIDS}.length+"${listOfIDS}${listOfIDS}${listOfIDS}${listOfIDS}");
      if(JSON.stringify(${listOfIDS}) !== 'undefined')
{
	//alert("Condition Satisfied 1");
...
function startTimer(duration, display) {
	...
	/* $(document).ready(function(){
		//Restrict the user to refresh
		function triggerOnSecondRefresh() {
			localStorage.setItem('refreshCount', 0);
			redirectToLogin();
		    console.log("This function is triggered on the second refresh.");
		}
		...
		if (localStorage.getItem('refreshCount')) {
		    ...
		    // Check if this is the second refresh
		    if (refreshCount === 2) {
		        // Trigger the function
		        triggerOnSecondRefresh();
		    }
		} else {
		    // Page is loaded for the first time
		    // Set refresh count to 1
