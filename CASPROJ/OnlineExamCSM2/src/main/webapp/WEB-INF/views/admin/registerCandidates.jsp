...
<title>Analysis Report For Registered Candidates</title>
...
<link rel="https://code.jquery.com/jquery-3.7.1.js">

<link rel="https://cdn.datatables.net/2.0.1/js/dataTables.js">
...
<body>
	<div class="card" id="registerCandidate">
		...
		<h3 class="card-title text-center text-dark">Register Candidates
			List</h3>

		<div class="card-header">
			<h4 class="text-center text-danger">Filter</h4>
			<form class="form-group" method="post"
				action="./searchRegisterCandidate">
				<div class="row">
      ...
      selected = 'selected' </c:if>>${pv.testTakerName}</option>
		...
		<div class="card-body">
			<table id="example" class="table table-bordered table-striped">
				...
				<tbody>
					<c:choose>
						<c:when test="${empty allRegisterCandidates}">
							<tr>
								<td colspan="9">No data available</td>
							</tr>
						</c:when>
						<c:otherwise>
							<c:forEach items="${allRegisterCandidates}" var="cand"
								varStatus="count">
								<tr>
									<td>${count.count}</td>
							...
							"buttons" : [ {
								...
								title : function() {
									...
									if (collegeName != '--select--'
											&& batchName != '--select--') {
										return 'Analysis Report of Registerd Candidate- '
												+ collegeName + ' - '
												+ batchName;
										...
										return 'Analysis Report - of Registered Candidates '
									}
								}
							} ]
						});
